...
const ResourcesAllocation = () => {
    ...
    const [newResource, setNewResource] = useState({
        ...
        unit: '',
        allocatedTo: '',
        completedDate: '', // ADDED: completedDate to state
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            fetchResources();
            setNewResource({ resourceName: '', name: '', type: '', description: '', quantity: 1, unit: '', allocatedTo: '', completedDate: '' }); // Clear completedDate too
            setIsFormOpen(false);
        } catch (err) {
    ...
    const handleEdit = (resource) => {
        ...
        setNewResource({ ...resource, completedDate: resource.completedDate ? new Date(resource.completedDate).toISOString().split('T')[0] : '' }); // Format date for input
        setIsFormOpen(true);
    };
    ...
    const handleApproveRequest = async (request) => {
        try {
            await axios.post(baseURL, {
                ...
                completedDate: request.completedDate // Send completedDate from request
            });
                ...
                <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    ...
                    {isFormOpen && (
                        <form onSubmit={editingResourceId ? handleUpdate : handleSubmit}>
                            ...
                            <div className="mb-4">
                                <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="completedDate">
                                    Needed/Completed Date:
                                </label>
                                <input
                                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 bg-white leading-tight focus:outline-none focus:shadow-outline"
                                    id="completedDate"
                            ...
                            <table className="min-w-full bg-white border border-gray-200">
                                <thead>
                                    <tr className="bg-gray-100">
                                        ...
                                        <th className="py-2 px-4 border-b">Needed Date</th> {/* ADDED Needed Date Header */}
                                ...
                                <tbody>
                                    {resourceRequests.map((request) => (
                                        <tr key={request._id}>
                                            ...
                                            <td className="py-2 px-4 border-b">{request.completedDate ? new Date(request.completedDate).toLocaleDateString() : 'N/A'}</td> {/* ADDED Needed Date Data */}
                            ...
                            <table className="min-w-full bg-white border border-gray-200">
                                <thead>
                                    <tr className="bg-gray-100">
                                        ...
                                        <th className="py-2 px-4 border-b"><Skeleton height={20} /></th>
                                        <th className="py-2 px-4 border-b"><Skeleton height={20} /></th>
                                        <th className="py-2 px-4 border-b"><Skeleton height={20} /></th> {/* Skeleton for new column */}
                                        <th className="py-2 px-4 border-b"><Skeleton height={20} /></th> {/* Skeleton for actions column */}
                                    </tr>
                                </thead>
                                <tbody>
                                    {Array.from({ length: 5 }).map((_, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="py-2 px-4 border-b"><Skeleton height={20} /></td>
                                            <td className="py-2 px-4 border-b"><Skeleton height={20} /></td>
                                            <td className="py-2 px-4 border-b"><Skeleton height={20} /></td> {/* Skeleton for new column */}
                                            <td className="py-2 px-4 border-b"><Skeleton height={20} /></td> {/* Skeleton for actions column */}
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            ...
                            <table className="min-w-full bg-white border border-gray-200">
                                <thead>
                                    <tr className="bg-gray-100">
                                        ...
                                        <th className="py-2 px-4 border-b">Unit</th>
                                        <th className="py-2 px-4 border-b">Allocated To</th>
                                        <th className="py-2 px-4 border-b">Assigned Date</th> {/* ADDED Assigned Date Header */}
                                        <th className="py-2 px-4 border-b">Needed Date</th>   {/* ADDED Needed Date Header */}
                                        <th className="py-2 px-4 border-b">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {resources.map((resource) => (
                                        <tr key={resource._id}>
                                            ...
                                            <td className="py-2 px-4 border-b">{resource.assignedDate ? new Date(resource.assignedDate).toLocaleDateString() : 'N/A'}</td> {/* ADDED Assigned Date Data */}
                                            <td className="py-2 px-4 border-b">{resource.completedDate ? new Date(resource.completedDate).toLocaleDateString() : 'N/A'}</td> {/* ADDED Needed Date Data */}
                                            <td className="py-2 px-4 border-b">
                                                <button
