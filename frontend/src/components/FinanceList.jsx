...
const FinanceList = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${baseURL}/get`, {
      params: { page: currentPage, limit: 10, department: 'finance' } // Pass pagination params
    })
    .then(response => {
  ...
  const handleDeleteUser = async (userId) => {
    try {
      ...
      if (response.status === 200) {
        ...
        // Optionally remove the user from the local state without re-fetching:
      ...
      try {
        ...
        for (let page = 1; page <= totalPages; page++) {
          const response = await axios.get(`${baseURL}/get`, {
            params: { page: page, limit: 10, department: 'finance' }
          });
        ...
        XLSX.writeFile(wb, 'users_backup.xlsx'); // Save the file
      } catch (error) {
