...
const DocumentStorage = () => {
    ...
    const [newDocument, setNewDocument] = useState({
        title: '',
        date: '',
        category: '', // Changed default to empty string
        status: 'Pending',
        description: '',
        file: null,
        attachmentName: null // Keep attachmentName for edit mode to send back to backend
    });
    ...
    useEffect(() => {
        if (editingDocumentId) {
            ...
            setNewDocument({
                ...
                description: '',
                file: null,
                attachmentName: null // Keep it for reset in edit cancel
            });
        }
    ...
    const fetchDocuments = async () => {
        try {
            ...
            toast.error("Error fetching documents"); // Toast for fetch error
        }
    };
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            const maxSizeMB = 10; // 10MB limit (Cloudinary Free plan)
            const maxSizeByte = maxSizeMB * 1024 * 1024;

            if (file.size > maxSizeByte) {
                toast.error(`File size exceeds the limit of ${maxSizeMB}MB. Please choose a smaller file.`); // Toast for file size error
                e.target.value = ''; // Clear the file input
                setNewDocument({ ...newDocument, file: null, fileBase64: null }); // Reset file state
            ...
            setNewDocument({ ...newDocument, file: null, fileBase64: null });
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!newDocument.title || !newDocument.date) {
            toast.error('Title and Date are required.'); // Toast for required fields
            return;
        }
        ...
        try {
            const payload = {
                ...
                fileName: newDocument.file ? newDocument.file.name : null, // Send filename if file is selected
            };

            if (isEditing) {
                payload.existingAttachmentName = newDocument.attachmentName; // Send existing URL for update if no new file
                await handleUpdateDocument(payload);
            } else {
                await axios.post(`${baseURL}/create`, payload);
                fetchDocuments();
                toast.success('Document created successfully!'); // Success toast for create
            }

            // Reset the form (keep attachmentName in reset for edit cancel)
            setNewDocument({
                title: '',
                date: '',
                ...
                file: null,
                fileBase64: null,
                attachmentName: isEditing ? newDocument.attachmentName : null // Keep attachmentName if editing and cancel
            });
            if (isEditing) setEditingDocumentId(null); // Exit edit mode after update/cancel
        } catch (error) {
            console.error("Error submitting document:", error);
            toast.error("Error submitting document"); // Error toast for create
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            fetchDocuments();
            toast.success('Document deleted successfully!'); // Success toast for delete
        } catch (error) {
            console.error("Error deleting document:", error);
            toast.error("Error deleting document"); // Error toast for delete
        }
    };
    ...
    const handleStatusChange = async (id, newStatus) => {
        try {
            ...
            fetchDocuments();
            toast.success(`Status updated to ${newStatus}!`); // Success toast for status update
        } catch (error) {
            console.error("Error updating status:", error);
            toast.error("Error updating status"); // Error toast for status update
        }
    };
    ...
    const handleCancelEdit = () => {
        ...
        setIsEditing(false); // Ensure isEditing is reset on cancel
        setNewDocument({ // Reset form but keep attachmentName if it existed before edit
            title: '',
            date: '',
            category: '', // Reset to empty string
            status: 'Pending',
            ...
            file: null,
            fileBase64: null,
            attachmentName: newDocument.attachmentName // Keep attachmentName from before edit
        });
    };
    ...
    const handleUpdateDocument = async (payload) => {
        try {
            await axios.put(`${baseURL}/update/${editingDocumentId}`, payload);
            fetchDocuments();
            setEditingDocumentId(null);
            setIsEditing(false); // Ensure isEditing is reset after update

            setNewDocument({ // Reset form but keep attachmentName for edit cancel
                title: '',
                date: '',
                category: '', // Reset to empty string
                status: 'Pending',
                ...
                file: null,
                fileBase64: null,
                attachmentName: null // Reset attachmentName after successful update
            });
            toast.success('Document updated successfully!'); // Success toast for update

        } catch (error) {
            console.error("Error updating document:", error);
            toast.error("Error updating document"); // Error toast for update
        }
    };
                                ...
                                <tr key={document._id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {document.attachmentName && ( // Check for attachmentName, not file.
                                            <a
                                                href={document.attachmentName}  // Use Cloudinary URL directly
                                                target="_blank"
                                            ...
                                            <div className="mt-2">
                                                ...
                                                {viewingDocument.attachmentName && ( // Check for attachmentName
                                                    <p>
                                                        <strong>File:</strong>
                                                        <a
                                                            href={viewingDocument.attachmentName} // Use Cloudinary URL directly
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="text-blue-500 hover:text-blue-700"
                                                        >
